...
void MD4Transform(unsigned long[4], unsigned char[64]);
void md4encode(unsigned char *, unsigned long *, unsigned short);
void md4decode(unsigned long *, unsigned char *, unsigned short);
...
{
  unsigned short i, index, partLen;
...
void otpmd4final(unsigned char *digest, struct otpmdx_ctx *context)
{
  ...
  otpmd4update(context, PADDING, padLen);

  /* Append length (before padding) */
  otpmd4update(context, bits, 8);
 ...
 */
void MD4Transform(unsigned long state[4], unsigned char block[64])
{
...
/* Encodes input (unsigned long) into output (unsigned char). Assumes len is
   a multiple of 4.
 */
void md4encode(unsigned char *output, unsigned long *input, unsigned short len)
{
  unsigned short i, j;
...
/* Decodes input (unsigned char) into output (unsigned long). Assumes len is
   a multiple of 4.
 */
void md4decode(unsigned long *output, unsigned char *input, unsigned short len)
{
  unsigned short i, j;
