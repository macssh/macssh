	      ...
	      struct lsh_string *key)
{
  ...
  if (SPKI_AUTHORIZE(self->db, subject, self->access))
      ...
      if (!self->sloppy)
	{
      ...
      if (!quiet_flag)
	{
	  extern short save_once_cancel1(const char *fingerprint);
	  extern short save_once_cancel2(const char *fingerprint);
	  /* Check if this host was known */
	  int found = 0;
	  int soc = 0;
	  struct lsh_string *fingerprint;
	  {
	    struct lsh_string *tmp = ssh_format("%lzknown_hosts", getenv("HOME"));
	    const char *s = lsh_get_cstring(tmp);
	    FILE *fd = fopen(s, "r");
	    char buf[256];
	    lsh_string_free(tmp);
	    if (fd == NULL)
	    ...
	    while ( fgets( buf, sizeof(buf) - 1, fd ) ) {
	      if ( (s = strstr( buf, "; ACL for host ") ) != NULL) {
		if ( !strncmp( s + 15, (char *)self->host->ip->data,
	      		     strlen((char *)self->host->ip->data)) ) {
	  	  found = 1;
	  	  break;
		}
	      }
	  ...
	  fingerprint
	    = hash_string(self->hash,
			  ...
			  1);
	  fingerprint = ssh_format("%lfxS", fingerprint);
	  if ( !found ) {
	    soc = save_once_cancel1(lsh_get_cstring(fingerprint));
	  } else {
	    soc = save_once_cancel2(lsh_get_cstring(fingerprint));
	  }
