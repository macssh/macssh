	      ...
	      struct lsh_string *key)
{
  ...
  if (SPKI_AUTHORIZE(self->db, subject, self->access))
      ...
      if (!self->sloppy)
	{
      ...
      if (!quiet_flag)
	{
	  extern short save_once_cancel1(char *fingerprint);
	  extern short save_once_cancel2(char *fingerprint);
	  /* Check if this host was known */
	  int found = 0;
	  int soc = 0;
	  struct lsh_string *fingerprint;
	  {
	    struct lsh_string *tmp = ssh_cformat("%lzknown_hosts", getenv("HOME"));
	  ...
	  fingerprint
	    = hash_string(self->hash,
			  ...
			  1);
	  fingerprint = ssh_cformat("%lfxS", fingerprint);
	  if ( !found ) {
	    soc = save_once_cancel1((char *)fingerprint->data);
	  } else {
	    soc = save_once_cancel2((char *)fingerprint->data);
	  }
